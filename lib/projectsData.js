// this array hold the projects
const projects = [
  // sample project data
  // {
  //   title: 'Project name here',
  //   description: 'Short description about the project here',
  //   techStack: 'React, NextJS, Javascript',
  //   image: '/thumb1.jpg',
  //   links: [
  //     {title: 'Live website', url: 'https://#'},
  //     {title: 'GitHub repo', url: 'https://#'},
  //   ],
  //   // optional: iOS/Android app store links, rendered as store logos
  //   stores: [
  //     {platform: 'ios', url: 'https://#'},
  //     {platform: 'android', url: 'https://#'},
  //   ]
  // },
  {
    title: 'Sijaismestari',
    description: 'Substitute staffing platform for Finnish education. Units post substitute jobs, qualified workers get matched and accept them in the app.',
    techStack: ['React Native', 'Expo', 'TypeScript', 'React', 'Django'],
    image: '/thumbnails/Sijaismestari-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://www.sijaismestari.fi/'},
    ],
    stores: [
      {platform: 'ios', url: 'https://apps.apple.com/fi/app/sijaismestari/id6784601798'},
      {platform: 'android', url: 'https://play.google.com/store/apps/details?id=com.nonitech.sijaismestari'},
    ]
  },
  {
    title: 'Netin Turva',
    description: 'Interactive digital security assessment and webstore for Netin Turva\'s cybersecurity subscriptions.',
    techStack: ['NextJS', 'React', 'TypeScript', 'Tailwind', 'PostgreSQL'],
    image: '/thumbnails/NetinTurva-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://www.netinturva.fi/'},
    ]
  },
  {
    title: 'Hobbly',
    description: 'Local activities platform',
    techStack: ['React', 'React Native', "Laravel", 'AWS'],
    image: '/thumbnails/Hobbly-thumbnail.webp',
    links: [
      {title: 'More Info', url: 'https://hobbly.app/'},
    ],
        stores: [
      {platform: 'ios', url: 'https://apps.apple.com/us/app/hobbly/id6746831794'},
      {platform: 'android', url: 'https://play.google.com/store/apps/details?id=com.hobbly.app'},
    ]
  },
  {
    title: 'Lumina',
    description: 'AI tarot reader that runs a fine-tuned LLM entirely in the browser via WebGPU — readings stay private on your device',
    techStack: ['NextJS', 'React', 'TypeScript', 'WebLLM', 'AI'],
    image: '/thumbnails/lumina-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://tarot.alextran.dev/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/lumina'},
    ]
  },
  {
    title: 'RPS League',
    description: 'Real-time match tracker and leaderboards for the Reaktor RPS Arena, with a 180,000+ match history archive',
    techStack: ['React', 'TypeScript', 'Vite', 'Tailwind', 'Shadcn UI'],
    image: '/thumbnails/RPS-League-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://rps.alextran.dev'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/RPS-League-Reaktor'},
    ]
  },
  {
    title: 'Unelma Browser',
    description: 'Open source contribution to a privacy-focused web browser built on Electron by Unelma Platforms',
    techStack: ['Electron', 'Javascript', 'Webpack'],
    image: '/thumbnails/unelma-browser-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://unelmas.com'},
      {title: 'GitHub repo', url: 'https://github.com/unelmacoin/unelma-browser'},
    ]
  },
  {
    title: 'Unelma Code Translate',
    description: 'Open source contribution to Unelma Platforms\' AI-powered code translator between programming languages',
    techStack: ['NextJS', 'TypeScript', 'OpenAI', 'Firebase'],
    image: '/thumbnails/unelma-code-translate-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://translate.u16p.com'},
      {title: 'GitHub repo', url: 'https://github.com/unelmacoin/unelma-code-translate'},
    ]
  },
  {
    title: 'Glimps Oy',
    description: 'Photobooth startup project',
    techStack: ['React', 'Python', "FastAPI", 'Docker'],
    image: '/thumbnails/Glimps-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://glimps.site/'},
    ]
  },
  {
    title: 'Druid.fi',
    description: 'Decoupled business website for Druid Oy Finland',
    techStack: ['React', 'Redux', 'TypeScript', 'Drupal'],
    image: '/thumbnails/Druid-website-thumbnail.webp',
    links: [
      // {title: 'Live website', url: 'https://druid-project.alextran.dev/'}, // backup for when client site is down
      {title: 'Live website', url: 'https://druid.fi/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/Druid_Partnering_Project'},
    ]
  },
  {
    title: 'Sauna Surfing',
    description: 'Full stack airbnb clone for sauna booking, built entirely by AI',
    techStack: ['AI', 'Lovable AI', 'Prompting', 'React'],
    image: '/thumbnails/SaunaSurfing-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://sauna-surfing-helsinki.lovable.app/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/sauna-surfing-helsinki'},
    ]
  },
  {
    title: 'ThriveConnect',
    description: 'Redefine Work Well-being. Challenge project for Junction Hackathon 2024 - Going Offline',
    techStack: ['React', 'Redux', 'Material UI','json-server', 'AI' ],
    image: '/thumbnails/ThriveConnect-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://junction2024.alextran.dev/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/junction_2024_challenge'},
    ]
  },
  {
    title: 'Countries App',
    description: 'Full-stack countries info app feature authentication and AI integration',
    techStack: ['React', 'Redux', 'Express','Bootstrap', 'Firebase', 'AI' ],
    image: '/thumbnails/countries-app-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://countries.alextran.dev'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/countries_app_react_bootstrap'},
    ]
  },
  {
    title: 'Portfolio v3',
    description: 'Version 3 of my personal portfolio',
    techStack: ['React', 'NextJS', 'Tailwind', 'Framer Motion'],
    image: '/thumbnails/portfolio-v3-thumbnail.webp',
    links: [
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/portfolio_v3'},
    ]
  },
  {
    title: 'Carwash.fi',
    description: 'Carwash Suomi franchise business website',
    techStack: ['Webflow', 'Javascript', 'Html', 'CSS'],
    image: '/thumbnails/carwash.fi-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://carwash.fi'},
    ]
  },
  {
    title: 'Travel buddy',
    description: 'Decoupled app providing destination itineraries, weather, maps, etc.',
    techStack: ['React', 'Javascript', 'Symfony', 'PHP'],
    image: '/thumbnails/travel-buddy-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://foremap.alextran.dev'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/fullstack_travel_buddy'},
    ]
  },
  {
    title: 'Symfony Social media',
    description: 'Self hosted, fullstack social media page built with Symfony',
    techStack: ['Symfony', 'PHP', 'Webpack', 'Tailwind'],
    image: '/thumb3.jpg',
    links: [
      {title: 'Live website', url: 'https://social-media.alextran.cloud'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/social_media_platform'},
    ]
  },
  {
    title: 'AI powered quiz app',
    description: 'Choose a topic, take a quiz and receive your score',
    techStack: ['NextJS', 'Typescript', 'Tailwind', 'OpenAI'],
    image: '/thumbnails/AI-quiz-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://ai-quiz.alextran.dev/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/AI-quiz-app'},
    ]
  },
  {
    title: 'URL Shortener',
    description: 'Provide users with a shortened URL',
    techStack: ['React', 'Vite', 'Javascript', 'Material UI'],
    image: '/thumbnails/URL-shortener-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://shorturl.alextran.dev'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/fullstack_url_shortener'},
    ]
  },
    {
    title: 'Portfolio v2',
    description: 'The second version of my personal portfolio',
    techStack: ['Javascript', 'Html', 'CSS'],
    image: '/thumbnails/portfolio-v2-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://alextrandev.github.io/portfolio_v1_and_v2/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/portfolio_v1_and_v2'},
    ]
  },
  {
    title: 'Pokedex',
    description: 'Browse, search and view info on over 500 Pokémon',
    techStack: ['Javascript', 'Html', 'CSS'],
    image: '/thumbnails/pokedex-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://alextrandev.github.io/pokedex/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/pokedex'},
    ]
  },
  {
    title: 'PHP Bookstore',
    description: 'A bookstore site built with vanilla PHP',
    techStack: ['PHP', 'Html', 'CSS'],
    image: '/thumbnails/PHP-bookstore-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'http://alextrandev.infinityfreeapp.com/bookstore_v2'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/php_bookstore_v2'},
    ]
  },
  {
    title: 'Starshop',
    description: 'E-commerce site for starships and starships repair',
    techStack: ['Symfony', 'PHP', 'Tailwind'],
    image: '/thumbnails/starshop-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://alextrandev.infinityfreeapp.com/starshop/public'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/starshop'},
    ]
  },
  {
    title: 'Animals app',
    description: 'Visitor can search, show animals info, mark favourites, give likes, etc.',
    techStack: ['React', 'Vite', 'Tailwind', 'RestAPIs'],
    image: '/thumbnails/animals-app-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://animals.alextran.dev/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/react_animals'},
    ]
  },
  {
    title: 'Message encryption',
    description: 'A message encryption/decryption program',
    techStack: ['Python', 'PyScript'],
    image: '/thumbnails/message-encryption-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://alextran.pyscriptapps.com/message-encryption/'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/python_message_encryption'},
    ]
  },
  {
    title: 'PHP Authentication system',
    description: 'An authenticator app feature account activation and password reset using email link verification',
    techStack: ['PHP', 'Html', 'CSS'],
    image: '/thumbnails/PHP-authentication-system-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'http://alextrandev.infinityfreeapp.com/PHP_authentication_system'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/php_authentication_system'},
    ]
  },
  {
    title: 'Symfony link page',
    description: 'Symfony practice project includes a palindrome check, calculator app and some sample pages',
    techStack: ['Symfony', 'PHP'],
    image: '/thumbnails/symfony-link-page-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'http://alextrandev.infinityfreeapp.com/symfony_link_page/public'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/symfony_link_page'},
    ]
  },
  {
    title: 'Country info',
    description: 'Display various info about all countries in the world using the REST Countries API',
    techStack: ['Javascript', 'Html', 'CSS'],
    image: '/thumbnails/countries-info-thumbnail.webp',
    links: [
      {title: 'Live website', url: 'https://alextrandev.github.io/country_info'},
      {title: 'GitHub repo', url: 'https://github.com/alextrandev/country_info'},
    ]
  },
]

// this function will split the projects array into groups of given number of elements
const projectsSlides = (numberOfProject) => {
// this array hold the slides
const slides = [];
// this loop break the projects array into groups of x items and add to the slide
for (let i = 0; i < Math.ceil(projects.length/numberOfProject); i++) {
 slides.push(projects.slice(i * numberOfProject, i * numberOfProject + numberOfProject));
}
return slides;
};

export default projectsSlides;